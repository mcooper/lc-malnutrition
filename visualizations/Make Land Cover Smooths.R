...
map <- data.frame(AEZ=c("s(natural,3.24):afr.arid.123", "s(natural,3.2):afr.forest.4", 
                  ...
                  Map=c("Arid", "Forest", "Northern Savanna", "Southern Savanna", "Highlands", "Northern Sub-Forest", 
                        "Southern Sub-Forest"))
...
ggplot(pltdat) + 
  ...
  scale_fill_manual(values = c('Arid'=rgb2(255, 225, 175),
                               ...
                               'Northern Sub-Forest'=rgb2(110, 175, 75),
                               'Southern Sub-Forest'=rgb2(55, 145, 130))) +
  theme_bw() + 
...
ggsave('C://Users/matt/lc-malnutrition-tex/AEZ_effects.png', height = 7, width = 7)

ggplot(pltdat %>% filter(Map %in% c("Northern Sub-Forest", "Southern Sub-Forest"))) + 
  geom_ribbon(aes(x=natural, ymin=min, ymax=max, fill=Map)) + 
  geom_line(aes(x=natural, y=fit)) + 
  geom_hline(aes(yintercept=0), linetype=3) + 
  facet_wrap(. ~ Map, ncol=2) + 
  scale_x_continuous(expand=c(0,0)) + 
  scale_fill_manual(values = c('Northern Sub-Forest'=rgb2(110, 175, 75),
                               'Southern Sub-Forest'=rgb2(55, 145, 130))) +
  theme_bw() + 
  labs(y='Coefficient for 24-Month SPEI',
       x='Percent of Nearby Landcover Uncultivated') + 
...
ggsave('C://Users/matt/lc-malnutrition-tex/AEZ_effects_sel.png', height = 3, width = 6)
